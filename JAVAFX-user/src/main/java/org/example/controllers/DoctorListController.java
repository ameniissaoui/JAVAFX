...
import org.example.services.PurgoMalumService;
...
public class DoctorListController {
    ...
    private void showReportDialog(Medecin doctor) {
        ...
        contentBox.getChildren().addAll(headerBox, instructionLabel, reasonField);
        ...
        // Add PurgoMalum validation when the report button is clicked
        reportButton.addEventFilter(ActionEvent.ACTION, event -> {
            // Check for profanity before closing the dialog
            String reportText = reasonField.getText();
            PurgoMalumService purgoMalumService = new PurgoMalumService();
            boolean hasProfanity = purgoMalumService.containsProfanity(reportText);

            if (hasProfanity) {
                // If inappropriate content is detected, show an ALERT and prevent dialog from closing
                Stage stage = (Stage) dialog.getDialogPane().getScene().getWindow();
                showAlert(stage, "warning", "Votre signalement contient un langage inapproprié et ne peut pas être soumis. Veuillez réviser votre texte.");
        ...
        dialog.showAndWait().ifPresent(reason -> {
            if (reason.trim().isEmpty()) {
                Stage stage = (Stage) doctorList.getScene().getWindow();
                showAlert(stage, "warning", "Veuillez fournir une raison pour le signalement.");
            } else {
    ...
    private void submitReport(Medecin doctor, String reason) {
        // Double-check for inappropriate content as a safeguard
        PurgoMalumService purgoMalumService = new PurgoMalumService();
        boolean hasProfanity = purgoMalumService.containsProfanity(reason);

        Stage stage = (Stage) doctorList.getScene().getWindow();

        if (hasProfanity) {
            // Show alert if inappropriate content is detected
            showAlert(stage, "warning", "Votre signalement contient un langage inapproprié et ne peut pas être soumis. Veuillez réviser votre texte.");
            return; // Stop here - do not register the report
        }

        // Only proceed with submission if content is appropriate
        int patientId = SessionManager.getInstance().getCurrentUser().getId();
    ...
    public void redirectToCalendrier(ActionEvent event) {
        SceneManager.loadScene("/fxml/patient_calendrier.fxml", event);
