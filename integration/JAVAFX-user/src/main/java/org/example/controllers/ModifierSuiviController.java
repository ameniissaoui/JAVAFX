...
public class ModifierSuiviController {
    ...
    // Ajoutez ces variables FXML
    @FXML
    private HBox dateInputBox;

    @FXML
    private HBox commentaireInputBox;
    ...
    public void initialize() {
        ...
        setupFocusListeners();
        // Planifie l'application du plein écran après que la fenêtre soit chargée
        Platform.runLater(() -> {
            if (commentaireArea.getScene() != null && commentaireArea.getScene().getWindow() != null) {
                Stage stage = (Stage) commentaireArea.getScene().getWindow();
                setFullScreen(stage);
            }
        });
        // Set up focus listeners for date picker
        datePicker.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue) {
                dateInputBox.getStyleClass().add("input-container-focused");
            } else {
                dateInputBox.getStyleClass().remove("input-container-focused");
            }
        });

        // Set up focus listeners for text area
        commentaireArea.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue) {
                commentaireInputBox.getStyleClass().add("input-container-focused");
            } else {
                commentaireInputBox.getStyleClass().remove("input-container-focused");
            }
        });
    }
    ...
    private void enregistrerModifications(ActionEvent event) {
        try {
            ...
            // Au lieu de fermer la fenêtre, appeler le callback pour revenir à la liste
    ...
    private void setupFocusListeners() {
        ...
        datePicker.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal) {
                dateInputBox.getStyleClass().add("focused");
            } else {
                dateInputBox.getStyleClass().remove("focused");
            }
        });
        // Pour le TextArea
        commentaireArea.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal) {
                commentaireInputBox.getStyleClass().add("focused");
            } else {
                commentaireInputBox.getStyleClass().remove("focused");
            }
        });
    }
    ...
    public void setFullScreen(Stage stage) {
        if (stage != null) {
            stage.setMaximized(true); // Mode maximisé (plein écran sans masquer les barres de tâches)
            // Si vous voulez un vrai plein écran (qui masque les barres de tâches):
