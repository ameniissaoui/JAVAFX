...
public class SuivServices implements Iservices<suivie_medical> {
    ...
    public void add(suivie_medical suivi) {
        ...
        try {
            ...
            String req = "INSERT INTO chrono.suivie_medical (date, commentaire, id_historique_id, user_id) VALUES (?, ?, ?, ?)";
    ...
    public void update(suivie_medical suivi) {
        String req = "UPDATE chrono.suivie_medical SET date=?, commentaire=?, id_historique_id=?, user_id=? WHERE id=?";
        try {
    ...
    public suivie_medical find(int id) {
        String req = "SELECT sm.*, u.*, m.* FROM chrono.suivie_medical sm " +
                "JOIN chrono.user u ON sm.user_id = u.id " +
                "LEFT JOIN chrono.medecin m ON u.id = m.user_id " +
    ...
    private void setStatementParameters(PreparedStatement stm, suivie_medical suivi) throws SQLException {
        ...
        // Handle user_id more explicitly
        if (suivi.getUser() == null || suivi.getUser().getId() <= 0) {
    ...
    private suivie_medical mapResultSetToSuiviMedical(ResultSet rs, historique_traitement historique) throws SQLException {
        ...
        medecin.setId(rs.getInt("user_id"));
        medecin.setNom(rs.getString("u.nom"));
        medecin.setPrenom(rs.getString("u.prenom"));
        medecin.setEmail(rs.getString("u.email"));
        medecin.setMotDePasse(rs.getString("u.motDePasse"));
        medecin.setDateNaissance(rs.getDate("u.dateNaissance"));
        medecin.setTelephone(rs.getString("u.telephone"));
        medecin.setBanned(rs.getBoolean("u.banned"));
        medecin.setImage(rs.getString("u.image"));
        medecin.setRole(rs.getString("u.role"));
        // Charger les champs spécifiques à Medecin depuis chrono.medecin
        medecin.setSpecialite(rs.getString("m.specialite"));
        medecin.setDiploma(rs.getString("m.diploma"));
        medecin.setIs_verified(rs.getBoolean("m.is_verified"));
