...
public class CommandeServices implements IServices<Commande> {
    ...
    public void addProduit(Commande commande) {
        ...
        String reqCommande = "INSERT INTO commande(nom, prenom, email, adresse, phone_number, stripe_session_id, payment_status, total_amount, utilisateur_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private void associateProductWithCommande(Produit produit, int commandeId) throws SQLException {
        String checkStockQuery = "SELECT stock_quantite FROM produit WHERE id = ?";
        ...
        String reqCartItem = "INSERT INTO cart_item(produit_id, commande_id, quantite) VALUES (?, ?, ?)";
    ...
    private void updateProductStock(int productId) throws SQLException {
        String reqUpdateStock = "UPDATE produit SET stock_quantite = stock_quantite - 1 WHERE id = ?";
    ...
    public void editProduit(Commande commande) {
        ...
        String req = "UPDATE commande SET nom=?, prenom=?, email=?, adresse=?, phone_number=?, stripe_session_id=?, payment_status=?, total_amount=?, utilisateur_id=? WHERE id=?";
        try (PreparedStatement stmt = cnx.prepareStatement(req)) {
    ...
    public Commande getCommandeByStripeSessionId(String stripeSessionId) {
        ...
        String req = "SELECT * FROM commande WHERE stripe_session_id = ?";
        try (PreparedStatement stmt = cnx.prepareStatement(req)) {
