...
public class CommandeServices implements IServices <Commande> {
    ...
    public void addProduit(Commande commande) {
        ...
        String reqCommande = "INSERT INTO sante.commande(nom, prenom, email, adresse, phone_number) VALUES (?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement stmtCommande = cnx.prepareStatement(reqCommande, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (generatedKeys.next()) {
                ...
                if (commande.getProduit() != null) {
                    ...
                    String reqCommandeProduit = "INSERT INTO sante.commande_produit(commande_id, produit_id) VALUES (?, ?)";
                    ...
                    String reqUpdateStock = "UPDATE sante.produit SET stock_quantite = stock_quantite - 1 WHERE id = ?";
    ...
    public List<Commande> showProduit() {
        ...
        String req = "SELECT c.*, cp.produit_id FROM sante.commande c " +
                "LEFT JOIN sante.commande_produit cp ON c.id = cp.commande_id";
        try {
