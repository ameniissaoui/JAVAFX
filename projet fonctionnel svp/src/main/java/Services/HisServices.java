...
public class HisServices implements Iservices<historique_traitement> {
    ...
    public void add(historique_traitement historiqueTraitement) {
        String req="INSERT INTO santee.historique_traitement (nom, prenom, maladie, description, type_traitement, bilan) VALUES (?,?,?,?,?,?)";
        try {
    ...
    public void update(historique_traitement historiqueTraitement) {
        String req = "UPDATE santee.historique_traitement SET nom=?, prenom=?, maladie=?, description=?, type_traitement=?, bilan=? WHERE id=?";
        try {
            PreparedStatement stm = cnx.prepareStatement(req);
            stm.setString(1, historiqueTraitement.getNom());
            stm.setString(2, historiqueTraitement.getPrenom());
    ...
    public void remove(historique_traitement historiqueTraitement) {
        String req = "DELETE FROM santee.historique_traitement WHERE id=?";
        try {
            PreparedStatement stm = cnx.prepareStatement(req);
    ...
    public historique_traitement find(int id) {
        String req = "SELECT * FROM santee.historique_traitement WHERE id = ?";
        try {
            PreparedStatement stm = cnx.prepareStatement(req);
            stm.setInt(1, id);
    ...
    public List<historique_traitement> afficher() {
        ...
        String req = "SELECT * FROM santee.historique_traitement";
        try {
    ...
    public historique_traitement getone() {
        String req = "SELECT * FROM santee.historique_traitement ORDER BY id DESC LIMIT 1";
        try {
            Statement stm = cnx.createStatement();
            ResultSet rs = stm.executeQuery(req);
            if (rs.next()) {
